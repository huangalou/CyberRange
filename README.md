...
- **`engine/`** — Python library + `cyberrange` CLI. Template-driven generation with realistic field distributions, rate control, time-window replay, multi-sink dispatch. 198 tests.
- **`api/` + `web/`** — FastAPI backend + Next.js UI: browse the catalog, preview samples, dispatch jobs, track history, reverse-lookup CVE → catalog → detection rule (`/vulnops`), CTI metrics dashboard (`/metrics`). 50 API tests.
